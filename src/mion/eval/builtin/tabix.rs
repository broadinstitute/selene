...
const OUTPUT_FILE_NAME_ARG: &str = "output_file_name";
const MISSES_FILE_NAME_ARG: &str = "misses_file_name";
...
impl Function for Tabix {
    ...
    fn call(&self, args_map: HashMap<Identifier, Value>) -> Result<Value, Error> {
        ...
        let output_file_name = get_string_arg(&args_map, &OUTPUT_FILE_NAME_ARG)?;
        let misses_file_name = get_string_arg(&args_map, &MISSES_FILE_NAME_ARG)?;
        let output_file =
            path_util::replace_file_name(&input_file, &output_file_name)?;
        let misses_file =
            path_util::replace_file_name(&input_file, &misses_file_name)?;
